...
$(window).load(function() {
	...
	if (progress > 50) {
		$('.progress .active2').css({
			'transform': 'rotate(' + (360 / 100) * (progress - 50) + 'deg)'
		})
		$('.progress .active2').css({
			'transform-origin': 'left center'
		})
		$('.progress .active2, .progress .active').css({
			'z-index': '300'
		})
	}
	...
	function reloader() {
		...
		$('#notif-content-wrapper').load(window.location.href + ' #notif-content', function() {
			...
			if (notifNumber > 0 && !$('#notif-menu').hasClass('notif-show')) {
				...
				if (notifNumber > 99) {
					$('#notification-icon #notif-number').css({
						'font-size': '14px'
					})
				}
				if ($('#notif-menu').hasClass('notif-show')) {
